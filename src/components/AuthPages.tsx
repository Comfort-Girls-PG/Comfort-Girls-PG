...
export default function AuthPages() {
 ...
 className="relative w-full max-w-md overflow-hidden bg-white border border-slate-200 rounded-3xl shadow-2xl z-10"
 >
 {/* Banner with logo & close */}
 <div className="relative p-6 bg-gradient-to-r from-primary/10 via-secondary/5 to-transparent flex items-center justify-between border-b border-slate-100 ">
 <div className="flex items-center gap-2">
 <div className="p-2 rounded-xl bg-primary text-white">
 <ShieldCheck className="w-5 h-5" />
 </div>
 <div>
 <h2 className="font-display font-semibold text-slate-800 leading-tight">
 ...
 <div className="p-6">
 {errorMsg && (
 <div className="mb-4 p-3.5 bg-red-50 border border-red-200 rounded-xl text-xs font-medium text-red-600 ">
 {errorMsg}
 </div>
 )}
 ...
 <div className="flex p-1.5 rounded-xl bg-slate-100 mb-6 font-display text-sm">
 <button
 ...
 {otpMode === "forgot" ? (
 <span>We sent a 4-digit passcode recovery PIN to <strong className="text-slate-800 ">{email || "your email address"}</strong>. Fill it in below to set your new passcode.</span>
 ) : (
 <span>We have dispatched a 4-digit validation pin to <strong className="text-slate-800 ">{email || "your email address"}</strong>. Please input the security passcode to complete registration.</span>
 )}
 </div>

 {debugOtp && (
 <div className="p-3 bg-pink-50/80 border border-pink-200 rounded-2xl text-xs text-pink-700 font-medium text-left space-y-2">
 ...
 className="text-[10px] font-bold px-2 py-1 bg-pink-100 hover:bg-pink-200 rounded-lg text-pink-700 transition-all cursor-pointer border border-pink-200 flex items-center gap-1"
 >
 ⚡ Autofill
 </button>
 </div>
 <p className="text-[11px] leading-relaxed text-slate-600 ">
 As requested, custom SMTP configuration is bypassed. Your live delivery security code is: <span className="font-mono text-base font-bold bg-white px-2 py-0.5 rounded-lg border border-pink-300 ml-1 select-all">{debugOtp}</span>
 </p>
 </div>
 )}

 {otpMode === "forgot" && (
 <div className="text-left py-2 border-y border-slate-100 space-y-2">
 ...
 className="w-full pl-10 pr-4 py-2.5 bg-slate-50 rounded-xl border border-slate-200 focus:border-primary focus:ring-1 focus:ring-primary outline-hidden text-sm text-slate-800 transition-colors"
 />
 </div>
 </div>
 )}
 ...
 className="w-14 h-14 text-center font-display font-bold text-2xl text-slate-800 bg-slate-50 border border-slate-200 rounded-2xl outline-hidden focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
 />
 ))}
 </div>
 ...
 Otp ticker: <strong className="text-slate-800 ">{otpTimer > 0 ? `00:${otpTimer < 10 ? '0' + otpTimer : otpTimer}` : "Expired"}</strong>
 </span>
 <button
 ...
 className="flex-1 py-3.5 border border-slate-200 text-slate-600 rounded-xl transition-all font-display text-sm font-semibold hover:bg-slate-50 disabled:opacity-50 cursor-pointer"
 >
 Back
 </button>
 <button
 ...
 <div className="p-4.5 bg-emerald-500/10 text-emerald-600 rounded-full">
 <Mail className="w-10 h-10 animate-bounce" />
 </div>
 </div>
 <div className="space-y-2">
 <h3 className="font-display font-semibold text-lg text-slate-800 ">Verify Your Email</h3>
 <p className="text-xs text-slate-500 leading-relaxed font-light">
 A secure authentication link was sent to <strong className="text-slate-800 ">{email}</strong>. Please check your inbox and click the verification link to confirm your email and activate your account.
 </p>
 </div>
 ...
 <a
 href={debugLink}
 className="block text-center font-bold px-3 py-2 bg-pink-100 hover:bg-pink-200 rounded-xl text-pink-700 transition-all border border-pink-200 break-all select-all mt-1"
 >
 ...
 type="button"
 onClick={() => setTab("login")}
 className="w-full py-3.5 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl transition-all font-display text-xs font-semibold cursor-pointer"
 >
 Back to Sign In
 </button>
 </motion.div>
 ...
 className="w-full pl-10 pr-4 py-3 bg-slate-50 rounded-xl border border-slate-200 focus:border-primary focus:ring-1 focus:ring-primary outline-hidden text-sm text-slate-800 transition-colors"
 />
 </div>
 </div>
 ...
 className="text-[11px] font-mono font-semibold text-slate-500 hover:text-primary transition-colors flex items-center justify-center gap-1.5 mx-auto cursor-pointer"
 >
 <span>🔧</span>
 ...
 {showDiagnostics && (
 <div className="mt-4 p-4 text-left bg-slate-50 border border-slate-200 rounded-2xl text-xs space-y-3">
 ...
 <span className="text-slate-600 font-sans">Backend Server:</span>
 <span className={`font-semibold flex items-center gap-1 ${diagResult.backendOk ? "text-emerald-600 " : "text-rose-600"}`}>
 {diagResult.backendOk ? "● Online & Connected" : "○ Unreachable (Offline)"}
 </span>
 </div>

 <div className="flex items-center justify-between border-b border-slate-100 pb-1.5">
 <span className="text-slate-600 font-sans">Supabase DB:</span>
 <span className={`font-semibold flex items-center gap-1 ${diagResult.dbConnected ? "text-emerald-600 " : "text-rose-600"}`}>
 {diagResult.dbConnected ? "● Connected (Active)" : "○ Offline / Not Connected"}
 </span>
 </div>

 <div className="flex items-center justify-between border-b border-slate-100 pb-1.5">
 <span className="text-slate-600 font-sans">Storage Mode:</span>
 ...
 <div className="flex items-center justify-between">
 <span className="text-slate-600 font-sans">Credentials host config:</span>
 <span className={`font-semibold ${diagResult.dbConfigured ? "text-emerald-600 " : "text-amber-600"}`}>
